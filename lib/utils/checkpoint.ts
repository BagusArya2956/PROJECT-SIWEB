...
export async function syncShipmentCheckpoints(snapshot: ShipmentCheckpointSnapshot) {
  ...
  for (const status of CHECKPOINT_SEQUENCE) {
    if (!completedStatuses.has(status) || existingStatuses.has(status)) {
      continue;
    }
  ...
  if (completedStatuses.has(CHECKPOINT_STATUS.SELESAI)) {
    await db.query(
      `
        ...
        SET
          shipment_status = 'SAMPAI',
          item_status = 'SELESAI',
          delivered_at = COALESCE(delivered_at, NOW())
        WHERE resi_code = $1
          AND shipment_status <> 'SAMPAI'
      `,
      [snapshot.resi]
    );
    ...
    await db.query(
      `
        INSERT INTO shipin_tracking_events (
          shipment_id,
          event_status,
          description,
          location_label,
